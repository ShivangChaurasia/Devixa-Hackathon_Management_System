    ...
    <div className="w-full bg-card border border-border rounded-2xl overflow-hidden shadow-xl">
      <div className="overflow-x-auto">
        <table className="w-full text-left text-sm text-white/80">
          <thead className="text-xs text-white/50 uppercase bg-card-secondary border-b border-border">
            <tr>
              {columns.map((col, idx) => (
      ...
      {pagination && data.length > 0 && (
        <div className="px-6 py-4 border-t border-border flex items-center justify-between bg-card-secondary/50">
          <span className="text-xs text-white/50">Showing 1 to {data.length} entries</span>
          <div className="flex gap-2">
            <button className="p-1 rounded bg-white/5 text-white/50 hover:text-white hover:bg-white/10 disabled:opacity-50">
              <ChevronLeft size={16} />
            </button>
            <button className="p-1 rounded bg-white/5 text-white/50 hover:text-white hover:bg-white/10 disabled:opacity-50">
