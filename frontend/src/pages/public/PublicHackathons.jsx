  ...
  return (
    <div className="min-h-screen bg-background text-white">
      ...
      <div className="max-w-[1200px] mx-auto px-4 md:px-8 py-12">
        {/* Header */}
        <div className="mb-10">
          <h1 className="text-4xl font-bold text-white mb-3">Browse Hackathons</h1>
          <p className="text-white/50 max-w-lg">Discover hackathons from universities, startups, and communities worldwide.</p>
        </div>
        ...
        <div className="flex flex-col md:flex-row gap-3 mb-8">
          <div className="flex-1 relative">
            ...
            <input
              ...
              className="w-full bg-card border border-border pl-12 pr-4 py-3 rounded-xl outline-none focus:border-accent-start transition-all text-white placeholder:text-white/40"
            />
          </div>
          <select
            value={themeFilter}
            onChange={(e) => setThemeFilter(e.target.value)}
            className="px-4 py-3 rounded-xl bg-card border border-border outline-none focus:border-accent-start text-white/70 transition-colors cursor-pointer"
          >
        ...
        {/* Results */}
        {loading ? (
          <div className="text-center text-white/50 py-20">Loading hackathons...</div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredHackathons.map((hackathon, index) => (
              ...
              >
                ...
                <div className="p-5">
                  <p className="text-xs text-white/40 mb-1">{hackathon.organization || 'Community'}</p>
                  <h3 className="text-lg font-semibold text-white mb-1 group-hover:text-accent-start transition-colors">{hackathon.title}</h3>
                  <p className="text-sm text-white/50 mb-4 line-clamp-2">{hackathon.tagline}</p>
                  <div className="flex flex-wrap gap-1.5 mb-4">
                    {hackathon.tags?.map(tag => (
                      <span key={tag} className="px-2 py-0.5 rounded-full bg-white/5 border border-white/10 text-[10px] text-white/60">{tag}</span>
                    ))}
                  </div>
                  <div className="flex items-center justify-between text-xs text-white/40 pt-4 border-t border-border">
          ...
          <div className="text-center py-20 bg-card border border-border rounded-2xl">
            ...
            <button
              ...
              className="px-4 py-2 rounded-lg bg-white/5 border border-border text-sm text-white/80 hover:bg-white/10 transition-colors"
            >
              Clear Filters
            </button>
          </div>
