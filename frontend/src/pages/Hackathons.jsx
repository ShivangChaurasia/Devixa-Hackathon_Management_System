    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row gap-4 mb-8">
        <div className="flex-1 relative">
          ...
          <input
            ...
            className="w-full bg-card border border-border pl-12 pr-4 py-3 rounded-xl outline-none focus:border-accent-start focus:ring-1 focus:ring-accent-start transition-all text-white placeholder:text-white/40"
          />
        </div>
        ...
        <select 
          ...
          className="px-4 py-3 rounded-xl bg-card border border-border outline-none focus:border-accent-start text-white/70 hover:text-white transition-colors cursor-pointer appearance-none"
        >
        ...
        <button className="flex items-center justify-center gap-2 px-6 py-3 rounded-xl bg-card border border-border hover:bg-white/5 transition-colors text-white font-medium">
          <Filter size={18} />
          More
        </button>
      </div>
      ...
      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[1, 2, 3].map((n) => (
            <GlassCard key={n} className="h-72 animate-pulse bg-white/5" hover={false} />
          ))}
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredHackathons?.length > 0 ? (
              ...
              >
                <GlassCard noPadding className="flex flex-col h-full group cursor-pointer" onClick={() => navigate(`/app/hackathons/${hackathon._id || hackathon.id}`)}>
                  ...
                  <div className="p-5 flex flex-col flex-1">
                    ...
                    <h3 className="text-xl font-bold text-white mb-2 group-hover:text-accent-start transition-colors">{hackathon.title}</h3>
                    <p className="text-sm text-white/50 mb-4 line-clamp-2 flex-1">{hackathon.tagline}</p>
                    
                    <div className="flex flex-wrap gap-1.5 mb-4">
                      {hackathon.tags?.slice(0,3).map(tag => (
                        <span key={tag} className="px-2 py-0.5 rounded-md bg-white/5 text-[10px] text-white/60 border border-white/10">
                          {tag}
                        </span>
                      ))}
                      {hackathon.tags?.length > 3 && (
                        <span className="px-2 py-0.5 rounded-md bg-white/5 text-[10px] text-white/60 border border-white/10">
                          +{hackathon.tags.length - 3}
                        </span>
                      )}
                    </div>
                    ...
                    <div className="flex items-center justify-between text-xs font-medium border-t border-border pt-4">
                      ...
                      <div className="text-white/40">{hackathon.participantsCount || 0} participants</div>
                    </div>
                  </div>
                </GlassCard>
              </motion.div>
            ...
            <div className="col-span-full py-20 text-center border border-dashed border-border rounded-2xl bg-white/[0.01]">
